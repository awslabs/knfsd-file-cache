...
variable "VERSION" {
  ...
  validation {
    ...
    error_message = "VERSION must be a valid semantic version 2.0.0 format. Example: \"1.1.0-alpha.4\"."
  }
}
...
variable "LOADBALANCER_IP" {
  description = "(Optional) The static private IPv4 address to use for the Internal Load Balancer when \"TRAFFIC_MODE = 'loadbalancer'\". If not specified, a random IP address will be assigned from the VPC Subnet. Default: \"null\"."
  type        = string
  nullable    = true
  default     = null
  validation {
