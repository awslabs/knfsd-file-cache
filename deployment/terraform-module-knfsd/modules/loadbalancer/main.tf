...
terraform {
  ...
  provider_meta "aws" {
    user_agent = [
      "knfsd-file-cache/modules/loadbalancer/1.1.0-alpha.26"
    ]
  }
}
...
data "dns_a_record_set" "knfsd_lb_ip" {
  host = aws_lb.knfsd_lb.dns_name
}
...
resource "aws_lb_listener" "knfsd_lb_listener" {
  for_each = var.NFS_PORTS

  load_balancer_arn = aws_lb.knfsd_lb.arn
  port              = each.value.port
  protocol          = "TCP_UDP"

  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.knfsd_lb_tg[each.key].arn
  }
...
resource "aws_lb_target_group" "knfsd_lb_tg" {
  for_each = var.NFS_PORTS
...
resource "aws_route53_zone" "lb" {
  count         = var.DNS_NAME == "" ? 1 : 0
  name          = local.dns_name
  comment       = "Internal DNS Zone for KNFSD Network Load Balancer"
  force_destroy = true
  vpc {
    vpc_id = local.vpc_id
  }
...
resource "aws_route53_record" "knfsd_lb_cname" {
  count   = var.DNS_NAME == "" ? 1 : 0
  name    = "nlb"
  type    = "CNAME"
  ttl     = 120
  records = [aws_lb.knfsd_lb.dns_name]
  zone_id = aws_route53_zone.lb[0].zone_id
}

# create a custom route53 CNAME record for the load balancer in pre-existing zone
resource "aws_route53_record" "knfsd_lb_cname_custom" {
  count   = var.DNS_NAME != "" ? 1 : 0
