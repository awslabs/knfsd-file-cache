...
variable "VERSION" {
  ...
  validation {
    ...
    error_message = "VERSION must be a valid semantic version 2.0.0 format. Example: \"1.1.0-alpha.28\"."
  }
}
...
variable "DNS_NAME" {
  description = "(Optional) The fully qualified DNS name (FQDN) to use for the KNFSD proxy cluster. Defaults to: \"nlb.{PROXY_BASENAME}.aws.internal.\" [Note: the trailing period is required]. Default: \"\"."
  type        = string
  nullable    = false
  default     = ""
  validation {
...
variable "VPC_CIDR" {
  description = "(Required) List of CIDR blocks to allow in security group rules."
  type        = list(string)
  nullable    = false
}
