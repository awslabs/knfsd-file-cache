...
output "dns_name" {
  ...
  value       = var.DNS_NAME == "" ? aws_route53_record.knfsd_lb_cname[0].fqdn : aws_route53_record.knfsd_lb_cname_custom[0].fqdn
}

output "ip_address" {
  description = "The private IP address of the KNFSD Network Load Balancer."
  value       = one(data.dns_a_record_set.knfsd_lb_ip.addrs)
}
...
output "lb_target_groups" {
  ...
  value       = { for k, v in aws_lb_target_group.knfsd_lb_tg : k => v.arn }
}
