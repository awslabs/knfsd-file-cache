...
locals {
  ...
  netapp_region  = var.NETAPP_SECRET_REGION != "" ? var.NETAPP_SECRET_REGION : var.REGION
}
...
data "aws_iam_policy_document" "ssm_parameter_store_policy_document" {
  statement {
    ...
    resources = ["arn:aws:ssm:${var.REGION}:${local.account_id}:parameter/knfsd/*"]
  }
}
...
# add custom IAM policy for db access if database == external & not deployed & config != ""
resource "aws_iam_role_policy_attachment" "knfsd_instance_role_custom_db_policy" {
