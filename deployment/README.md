...
| Variable                  | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Required                                                                             | Default |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ | ------- |
| `EXPORT_MAP`              | A list of NFS Exports to mount from source filer and re-export in the format `<SOURCE_IP/DNS>;<SOURCE_EXPORT>;<TARGET_EXPORT>`.<br><br> For example to mount `10.100.100.1/export` from source filer and re-export as `10.100.100.1/reexport` you would set the `EXPORT_MAP` variable to `10.100.100.1;/export;/reexport`.<br><br>You can specify multiple re-exports using a comma, for example `10.100.100.1;/assets;/assetscache,10.100.100.1;/textures;/texturescache`.<br><br>An `<optional>` 4th field can be used to override the filesystem type of the mount, for example `10.100.100.1;/export;/reexport;efs`. Supported filesystem types: `nfs` (default), `efs`, `s3files`. | `EXPORT_MAP`, `EXPORT_HOST_AUTO_DETECT` or NetApp Auto-Discovery must be configured. | N/A     |
...
If using the NetApp Exports Auto-Discovery feature, please also read the [NetApp Exports](docs/netapp.md) and [NetApp ShowMount Tool](../image/resources/netapp-exports/README.md) docs.
...
| Variable                     | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Required | Default        |
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | -------------- |
| `PROXY_BASENAME`             | Prefix used to name AWS resources. Every deployment in an AWS account *MUST* be given a unique basename to avoid conflicts (some of the resources created must have a globally unique name within an AWS account).                                                                                                                                                                                                                                             | False    | `knfsd`        |
| `VPC_CIDR`                   | List of CIDR blocks to use in security group rules. If empty, the primary VPC CIDR block is used. For secondary VPC CIDRs, you must explicitly provide the full list.                                                                                                                                                                                                                                                                                          | False    | `[]`           |
| `EXPORT_CIDR`                | List of CIDR blocks to use in NFSD `/etc/exports.d/knfsd.exports` file. If empty, the primary VPC CIDR block is used. For secondary VPC CIDRs, you must explicitly provide the full list.                                                                                                                                                                                                                                                                      | False    | `[]`           |
| `PROXY_AMI`                  | The AMI ID of the KNFSD image, built by Packer. Must match the architecture of `INSTANCE_TYPE`.                                                                                                                                                                                                                                                                                                                                                                | True     | N/A            |
| `PROXY_AMI_OWNERS`           | List of AMI owners to limit AMI search. Valid values: an AWS "account ID", "self" (the current account), or an AWS owner alias ("amazon", "aws-marketplace").                                                                                                                                                                                                                                                                                                  | False    | `"self"`       |
...
| `VFS_CACHE_PRESSURE`         | The value to set for `vfs_cache_pressure` rule.                                                                                                                                                                                                                                                                                                                                                                                                                | False    | `1`            |
| `READ_AHEAD`                 | The NFS readahead value in bytes, applied via nfsrahead udev rule in `/etc/nfs.conf.d/knfsd.conf`. Applies to all NFS mounts. EFS mounts use their own readahead via efs-utils.                                                                                                                                                                                                                                                                                | False    | `8388608`      |
...
### NFS Kernel Client Options

| Variable                     | Description                                                                                                                                                                        | Required | Default |
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------- |
| `TCP_SLOT_TABLE_ENTRIES`     | The initial number of RPC slot table entries for TCP connections to the source NFS server. Controls how many simultaneous RPC requests the proxy can send to the source filer.     | False    | `128`   |
| `TCP_MAX_SLOT_TABLE_ENTRIES` | The maximum number of RPC slot table entries for TCP connections to the source NFS server. Sets the upper limit on concurrent RPC requests the proxy can send to the source filer. | False    | `128`   |
...
| `SVC_RPC_PER_CONNECTION_LIMIT` | The number of RPC requests that the server will process in parallel from a single connection. The default value is `0` (no limit).                                                                                                                                                                                                                    | False    | `0`           |
