...
* The KNFSD proxy cluster can be manually resized using Terraform (recommended), AWS Console or the AWS CLI.

* Cannot resize the KNFSD proxy cluster while it has active NFS clients. This is because the clients will remain connected to the same IP.
  ...
  Scaling up the cluster *will not* redistribute existing clients to the new instances. When new clients are added, they will be distributed evenly across all instances in the cluster. This will result in the original instances being over-utilized, while the new instances are under-utilized.

* Auto scaling is not currently supported (`ENABLE_KNFSD_AUTOSCALING`).
...
Because the Auto Scaling Group (ASG) uses stateful IPs, the new instance will have the same IP as the original instance. The clients should reconnect to the new instance.

## Network Load Balancer

`TRAFFIC_MODE = "loadbalancer"`

This Terraform child module uses a [Network Load Balancer](https://docs.aws.amazon.com/elasticloadbalancing/latest/network/network-load-balancers.html) to distribute NFS client traffic between the proxy instances.

The NLB provides a single load balancer IP for the KNFSD proxy cluster. Clients connect to the load balancer IP, and the load balancer distributes the client connections between all the proxy instances.

Before using this option there are two things that need to be considered:

* Failure handling if a proxy instance is replaced.
* Cost of egress traffic

### Limitations: NLB

When scaling up an Auto Scaling Group (ASG) with connected clients, due to the long lived persistent connections the traffic *will not* be redistributed to the new instances. This is identical to the failure handling, only instead of replacing a failed instance, new instances are being added.

### Failure Handling: NLB
...
There is a [charge for number of bytes processed](https://aws.amazon.com/elasticloadbalancing/pricing) by the Network Load Balancer in GBs.
