...
| **knfsd/fscache/extents/max**                   | Highest extent count of any single FS-Cache backing file. SEEK_HOLE cost is proportional to this value.         |
| **knfsd/fscache/extents/mean_bytes**            | Mean extent length across all FS-Cache backing files. A value below the NFS rsize indicates poor coalescing.    |
| **knfsd/fscache/extents/unwritten_bytes**       | Capacity allocated in unwritten extents, reserved ahead of use by the XFS extent size hint.                     |
| **knfsd/fscache/fragmentation/scrape_duration** | Time taken to collect the FS-Cache fragmentation metrics. Used to tune the collection interval.                 |
