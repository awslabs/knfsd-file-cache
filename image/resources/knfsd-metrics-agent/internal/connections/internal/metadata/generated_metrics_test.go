		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, mi := range allMetricsList {
				...
				case "nfs.clients":
					...
					assert.Equal(t, "The number of unique NFS client IP addresses connected to the KNFSD filer (in any connected state)", mi.Description())
				...
				case "nfs.connections":
					...
					assert.Equal(t, pmetric.MetricTypeGauge, mi.Type())
					assert.Equal(t, 1, mi.Gauge().DataPoints().Len())
					assert.Equal(t, "The number of active (ESTAB) NFS connections to the KNFSD filer (1-16 per client, used for autoscaling)", mi.Description())
					assert.Equal(t, "{count}", mi.Unit())
					dp := mi.Gauge().DataPoints().At(0)
					assert.Equal(t, start, dp.StartTimestamp())
					assert.Equal(t, ts, dp.Timestamp())
