	...
	defer func() {
		for _, f := range files {
			...
			if err := f.Close(); err != nil {
				log.Warn.Printf("Error closing activation file: %v", err)
			}
		}
	}()
...
func newServerFromFile(f *os.File) (*server, error) {
	...
	if !ok {
		if closeErr := l.Close(); closeErr != nil {
			log.Warn.Printf("Error closing invalid file listener: %v", closeErr)
		}
	...
	if l.Addr().Network() != "unixpacket" {
		if closeErr := l.Close(); closeErr != nil {
			log.Warn.Printf("Error closing invalid network listener: %v", closeErr)
		}
		return nil, ErrInvalidNetwork
	}
...
func (c *connection) Serve() {
	...
	// There is a race condition between setting inShutdown, closing
	...
	for !c.shuttingDown() {
		...
		if n > PacketMaxLength {
			...
			err = c.writeError("message truncated")
			if err != nil {
				log.Error.Printf("[%d] write error: %s", c.id, err)
			}
			continue
		}
...
func (c *connection) Close() error {
	// Close the connection and cancel any commands that are in progress.
