...
This differs from other commands such as `stat`. Running a `stat` on a directory will only issue a `GETATTR` if the cached info has expired.

When querying directly to the source server, the source will reply to the `GETATTR` with the latest metadata.

When querying via a proxy, the proxy only sees a `GETATTR` request. The proxy does not know the client's context. So the proxy does not know if the `GETATTR` is for an `ls` command or a `stat` command. As such the proxy always responds to a `GETATTR` request from the cache.
...
This does not result in extra `READDIR` operations. If `GETATTR` indicates that proxy's cache is still valid (based on the directory's `mtime`), the proxy can respond to the `READDIR` from the proxy's cache.
